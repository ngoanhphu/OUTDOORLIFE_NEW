...
<div id="container">
  <div class="main_content_area">
    <div class="album py-5 bg-body-tertiary">
      <div class="container">
        <div class="card-header my-3">Owner Booked Information</div>
        <c:forEach var="owner" items="${ownerReportReport}">
          <div class="campsite-item" onclick="selectOwner(${owner.ownerId})">
            <img class="fixed-rectangle" src="${owner.image}" alt="Camp image">
            <div>
              <strong>${owner.ownerName}</strong><br>
              Ngày Sinh: ${owner.dob}<br>
              Địa chỉ: ${owner.address}<br>
              Giới tính: ${owner.gender}<br>
              Nghề nghiệp: ${owner.occupation}
            </div>
          </div>
        </c:forEach>
      </div>
...
<script>
  function selectOwner(ownerId) {
    console.log("Campsite_id: ", ownerId);
    const reportType = 'owner';
    window.location.href = "/login_war/getCampsiteDetail?ownerId=" + ownerId + "&reportType=" + reportType;
  }
</script>
</body>
